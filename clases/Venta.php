...
class Venta{
            public function save($clientee,$total,$tipo,$numero,$vendedor)
        {
            $c = new Conexion();
			$conexion = $c->conectar();
			$cliente = $c->test_input($clientee);
            $fecha = date('y-m-d');
			$sql = "INSERT INTO ventas(cliente,fecha,estado,total,tipo,numero,vendedor) values('$cliente','$fecha','activo','$total','$tipo','$numero','$vendedor')";
            ...
            public function save_detalle()
        {
            ...
            {
                $d = explode("||",$datos[$i]);
                $sql = "INSERT into detalle_ventas(id_ventas,id_productos,cantidad,precio,importe,descuento,preciodescuento)
                values('$id','$d[7]','$d[2]','$d[1]','$d[4]','$d[5]','$d[6]')";
                $this->baja_stock($d[2],$d[7]);
    ...
    public function mostrar_porid($id)
    {
            $c = new Conexion();
			$conexion = $c->conectar();
			$sql = "select id_venta,cliente,fecha,total,tipo,numero,vendedor from ventas where id_venta = '$id'";
			$result = mysqli_query($conexion,$sql);
    ...
    public function traer_detalles($id)
    {
            $c = new Conexion();
			$conexion = $c->conectar();
			$sql = "select de.cantidad,p.despacho,p.nombre,de.precio,de.importe,de.descuento,de.preciodescuento from detalle_ventas as de
